...
2. **The Dynamic Phase (The Real-Time Delta):** As the shooter twists the physical aperture, shutter, or ISO dials, the engine **does not redraw** the complex histogram. Instead, it feeds the single locked peak value into the lightweight, additive APEX math engine ($y_\text{peak} = e_\text{peak} - av + tv + sv_{\text{raw}}$).
